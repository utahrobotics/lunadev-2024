...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Train { learning_rate, hidden_size } => {
            let device = burn::backend::wgpu::WgpuDevice::default();
            train::<MyAutodiffBackend>(
                "/tmp/smooth_diff_drive",
                TrainingConfig::new(ModelConfig::new(hidden_size), AdamConfig::new().with_grad_clipping(Some(GradientClippingConfig::Value(0.25)))).with_learning_rate(learning_rate),
                device,
            );
        }
        Commands::Test => {
        ...
        Commands::Sample => {
            ...
            db.query_row("SELECT * FROM train ORDER BY RANDOM() LIMIT 1;", (), |row| {
