...
use unros_core::{
    anyhow, default_run_options, log::info, logging::dump::{DataDump, ScalingFilter, VideoDataDump}, pubsub::Subscriber, start_unros_runtime, tokio, Node
};

fn main() -> anyhow::Result<()> {
    start_unros_runtime(|mut app| async {
        ...
        let camera_count = discover_all_cameras()?.map(|mut x| x.get_intrinsics().ignore_drop()).count();
