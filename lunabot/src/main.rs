...
use unros_core::{
    anyhow, default_run_options,
    log::info,
    logging::dump::{DataDump, ScalingFilter, VideoDataDump},
    pubsub::Subscriber,
    start_unros_runtime, tokio, Node,
};
...
fn main() -> anyhow::Result<()> {
    start_unros_runtime(
        |mut app| async {
            ...
            let mut costmap_writer = VideoDataDump::new_file(
                80,
                80,
                720,
                720,
            ...
            let mut navigator =
                DirectPathfinder::new(robot_base_ref.clone(), costmap.get_ref(), 0.5, 0.15);
