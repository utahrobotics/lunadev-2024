...
pub struct Autonomy {
    arm_pub: Publisher<ArmParameters>,
        ...
        loop {
            match self.action_sub.recv().await {
                AutonomyAction::Dig => {
                    ...
                    lower_bucket_trans.set_transition(move |ret| ret.map(|_| drive_forward.clone()));
                    drive_forward_trans.set_transition(move |ret| ret.map(|_| raise_bucket.clone()));
