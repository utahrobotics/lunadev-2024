use std::{
    array, collections::HashMap, hash::{BuildHasherDefault, DefaultHasher}, io::stdin, num::NonZeroUsize, sync::{Arc, OnceLock}, time::Duration
};

use apriltag::AprilTagDetector;
use image::DynamicImage;
use opencv::{calib3d::{calibrate_camera, find_chessboard_corners, get_optimal_new_camera_matrix, CALIB_CB_ADAPTIVE_THRESH, CALIB_CB_NORMALIZE_IMAGE}, core::{Mat, MatTraitConst, Point3f, Rect, Size, TermCriteria, Vector}, imgproc::corner_sub_pix, types::{VectorOfMat, VectorOfPoint2f, VectorOfPoint3f, VectorOfVec3d}};
use rig::Robot;
use serde::{Deserialize, Serialize};
use serde_json::{from_reader, to_string_pretty};
use unros::{anyhow, log, pubsub::{Publisher, Subscriber, Subscription}, setup_logging, tokio::{self, task::JoinHandle}, Application};
...
pub async fn interactive_examine(app: &mut Application, accept_sub: impl FnOnce(Subscription<Arc<DynamicImage>>), camera_name: String) {
    ...
    let (mut elements, _) = rig.destructure::<BuildHasherDefault<DefaultHasher>>(["camera"]).unwrap();
    ...
    app.add_task(
        move |context| async move {
            ...
            let mut camera_info = CameraInfo { width: img.width(), height: img.height(), fps: None, focal_length_px: None, distortion_data: None };
            ...
            if chessboard {
                ...
                let new_camera_matrix = get_optimal_new_camera_matrix(&camera_matrix, &dist_coeffs, img_size, 1.0, img_size, Some(&mut roi), false).expect("Failed to execute get_optimal_new_camera_matrix");
            ...
            if let Some(FocalLengthEstimate { tag_distance, width, id }) = focal_length_estimate {
                ...
                loop {
                    ...
                    if (new_length - length).abs() < 1.0 {
                        ...
                        if close_enoughs >= 5 {
                            ...
                            camera_info.focal_length_px = Some(NonZeroUsize::new(new_length.round() as usize).unwrap());
                        }
                    } else {
                        close_enoughs = 0;
                    }
