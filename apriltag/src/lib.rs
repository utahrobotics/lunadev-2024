...
use nalgebra::{Isometry3, Point3, Scale3, UnitQuaternion, Vector3};
...
impl Display for PoseObservation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let (mut r, mut p, mut y) = self.pose.rotation.euler_angles();
        r *= 180.0 / PI;
        p *= 180.0 / PI;
        y *= 180.0 / PI;
        write!(f, "Observer pos: ({:.2}, {:.2}, {:.2}), roll: {r:.0}, pitch: {p:.0}, yaw: {y:.0}, margin: {:.0}", self.pose.translation.x, self.pose.translation.y, self.pose.translation.z, self.decision_margin)
    }
}
        ...
        let _ = tokio_rayon::spawn(move || {
            ...
            loop {
                ...
                for detection in detector.detect(&img) {
                    ...
                    let scale = Scale3::new(1.0, -1.0, -1.0);
                    let mut robot_pose = tag_pose.inverse();
                    robot_pose.translation.vector = scale * robot_pose.translation.vector;
                    robot_pose.rotation = UnitQuaternion::from_scaled_axis(scale * robot_pose.rotation.scaled_axis());

                    robot_pose.translation.vector += known.pose.translation.vector;
                    robot_pose.rotation = known.pose.rotation * robot_pose.rotation;
                    let velocity;
